...
#'  non-integer.
#' @param alpha The concentration parameter for the Pitman-Yor Process prior.
#' @param d The concentration parameter for the Pitman-Yor Process prior.
...
#' @export
mpy <- function(data, k, alpha = 1, d = 1, mu = 21, tau = 25, s = 4, S = 2,
                c = 2, C = 4, a = 21, A = 21, w = 1, W = 100,
                fix_a = TRUE, fix_m = FALSE, fix_t = FALSE,
                burn = 1000, thin = 150) {
  if (!fix_a) {
    stop('Must fix_a in PY')
  }
  out <- mpy_cpp(data, k, d, alpha, mu, tau, s, S, c, C, a, A, w, W,
                 fix_a, fix_m, fix_t, burn, thin)
  out[[3]] <- list(data = data, k = k, alpha = alpha, d = d, mu = mu, tau = tau,
  ...
  out$phi <- phi
  class(out) <- c('mdpolya_result', 'mpy')
  return(out)
}
