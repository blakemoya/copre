...
algorithm. Also included are a Gibbs sampler for the marginal **M**ixture of
**D**irichlet **P**rocess (MDP) model and an extension to include full
uncertainty quantification via a new Polya completion algorithm for the MDP. The
CopRe and Polya samplers generate random nonparametric distributions as output,
leading to complete nonparametric inference on posterior summaries. Routines for
...
which can then be used to estimate the clusters in the data using, for example,
k-means.
